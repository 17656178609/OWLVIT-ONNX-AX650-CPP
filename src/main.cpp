...
int main(int argc, char *argv[])
{
    ...
    // save text_features to bin file
    for (size_t i = 0; i < texts.size(); i++)
    {
        std::string save_path = texts[i] + ".bin";
        printf("{\n    \"test\": \"%s\",\n", texts[i].c_str());
        printf("    \"texts_feature\": \"%s\",\n    \"input_ids\":[", save_path.c_str());
        for (size_t j = 0; j < input_ids[i].size(); j++)
        {
            printf(" %ld", input_ids[i][j]);
            if (j != input_ids[i].size() - 1)
            {
                printf(",");
            }
        }
        ...
        outfile.open(save_path, std::ios::binary);
        if (!outfile.good())
        {
            ALOGE("");
            return -1;
        ...
        outfile.write((const char *)text_features[i].data(), text_features[i].size() * sizeof(float));
